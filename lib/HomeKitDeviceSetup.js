...
Service.prototype.updateUsingHSReference = function(value) 
{
	...
	switch (that.config.type) 
	{
		...
		default: 
		{
			...
			{
				that.log(yellow("WARNING: adding unspecified device type with HomeSeer reference " + that.config.ref + ". Defaulting to type Lightbulb. "));
				that.log(yellow("Future versions of this plugin may require specification of the type of each device."));
				that.log(yellow("Please update your config.json file to specify the device type."));
			}
