...
checkConfig = function(config, allHSDevices)
{
	...
	{ 
		...
		{
			this.log(green("Checking config.json configuration data for accessory with initial data: \n") + cyan(JSON.stringify(config.accessories[i]) ));
			...
			// If type is undefined, default to a lightbulb!
			...
			this.log(green("Full property information updated to: \n") + cyan(JSON.stringify(config.accessories[i]) ));	
		}	
