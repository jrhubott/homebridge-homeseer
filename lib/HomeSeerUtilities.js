...
checkConfig = function(config, allHSDevices)
{
	...
	{ 
		...
		switch(config.accessories[i].type)
		{
			...
			console.log(cyan("*Debugging: Entering Lightbulb / Fan case"));
				...
				if (config.accessories[i].can_dim === undefined)
					{
						config.accessories[i].can_dim = true; // default to true
						console.log(cyan("*Debugging: Initializing can_dim to true"));
						console.log(cyan("*Debug: Device Type string: " + CurrentHomeSeerDevice.device_type_string));
						if ( CurrentHomeSeerDevice.device_type_string && (CurrentHomeSeerDevice.device_type_string == "Z-Wave Switch Binary"))
						{
							console.log(magenta("*Debugging: Setting can_dim to false"));
			...
			if (config.accessories[i].can_dim == true) {
				...
				{
					that.log(chalk.magenta.bold("* Warning * - Check can_dim setting for device named: " + config.accessories[i].name + ", and HomeSeer reference: " + config.accessories[i].ref ));
					that.log(chalk.magenta.bold("HomeSeer reports model type: " + CurrentHomeSeerDevice.device_type_string + ", which typically does not provide variable level adjustment."));
				}
		...
		let thisConfig = Object.getOwnPropertyNames(config.accessories[i]);
		console.log(magenta("Debug: config.accessories[i]: " + thisConfig));
		...
		//	Make sure the specified HomeSeer device references are all numbers
			...
			{
				console.log(magenta(config.accessories[i].type));
				error =  red(
						"config.json error for device name: " + config.accessories[i].name 
