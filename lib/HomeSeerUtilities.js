...
checkConfig = function(config)
{
	...
	{ 
		...
		//	Make sure the specified HomeSeer device references are all numbers
			...
			if((config.accessories[i].OpenCloseOnly != null) && (typeof(config.accessories[i].OpenCloseOnly) == "string"))
			{
				switch(config.accessories[i].OpenCloseOnly.toLowerCase())
				{
					case("0"):
					case("true"):
					{
						config.accessories[i].OpenCloseOnly = true;
						break;
					}
					case("1"):
					case("false"):
					{
						config.accessories[i].OpenCloseOnly = false;
						break;
					}
					default:
					{
						var error = chalk.red.bold('HomeBridge config.json file error - Incorrect OpenCloseOnly setting. Must be one of "true" or "false" You have setting of: "' 
								+ config.accessories[i].OpenCloseOnly + '" for WindowCovering device with name ' + config.accessories[i].name
								+', and HomeSeer reference of: ' + config.accessories[i].ref)
						throw new SyntaxError(error);
					}
				} 
